...
import com.revy.example.jwt.payload.JwtTokenHash;
...
public class JwtSessionService implements JwtTokenBlacklist {
    ...
    public JwtSessionService(JwtTokenProvider jwtTokenProvider,
    ...
    public JwtSession issue(JwtPrincipal principal) {
        ...
        saveRefreshToken(refreshToken, refreshClaims);
     ...
     */
    public JwtSession refresh(RefreshTokenRequest request, PrincipalType expectedPrincipalType) {
    ...
    private void saveRefreshToken(String refreshToken, Claims claims) {
        String principalType = claims.get(JwtConstants.PRINCIPAL_TYPE_CLAIM, String.class);
        log.info("principalType: {}, claims.getSubject(): {}", principalType, claims.getSubject());
        JwtPrincipal principal = jwtPrincipalResolver.resolve(principalType, Long.valueOf(claims.getSubject()))
                                                     .orElseThrow(() -> new BusinessException(ErrorCode.INVALID_TOKEN));
